...
#define UPIMAGE ([UIImage imageNamed:@"image4"])
#define DOWNIMAGE ([UIImage imageNamed:@"image3"])
...
-(void)drawMark
{
    ...
    if (markCenter.x<=centerX&&markCenter.y<=centerY)//左上
    {
        ...
        {
            [markImageView setFrame:CGRectMake(showLocationRect.origin.x+showLocationRect.size.width/2, showLocationRect.origin.y+showLocationRect.size.height, markImageView.frame.size.width, markImageView.frame.size.height)];
            ...
            CGSize size = [markTextView sizeThatFits:CGSizeMake(width, height)];
            if ([[PronetwayYunFoodHandle shareHandle].caiPinRight isEqualToString:@"right"]) {
                
                [markTextView setFrame:CGRectMake(width, markImageView.frame.origin.y+markImageView.frame.size.height-markTextView.font.pointSize, size.width, size.height)];

            }else {
            
                [markTextView setFrame:CGRectMake(markImageView.frame.size.width, markImageView.frame.origin.y+markImageView.frame.size.height-markTextView.font.pointSize, size.width, size.height)];
            }
            [self addSubview:markTextView];
        }
    }
    else if (markCenter.x>=centerX&&markCenter.y<=centerY)//右上
    {
        ...
        if (left>=bottom)//左侧
        {
            [markImageView setFrame:CGRectMake(showLocationRect.origin.x-markImageView.frame.size.width/2, showLocationRect.origin.y, markImageView.frame.size.width, markImageView.frame.size.height)];
            [markImageView setImage:DOWNIMAGE];
            [markImageView setTransform:CGAffineTransformRotate(CGAffineTransformIdentity, M_PI_4)];
            [self addSubview:markImageView];
        ...
        {
            [markImageView setFrame:CGRectMake(showLocationRect.origin.x+showLocationRect.size.width/2-markImageView.frame.size.width, showLocationRect.origin.y+showLocationRect.size.height, markImageView.frame.size.width, markImageView.frame.size.height)];
            [markImageView setImage:UPIMAGE];
    ...
    else if (markCenter.x<=centerX&&markCenter.y>=centerY)//左下
    {
        ...
        {
            [markImageView setFrame:CGRectMake(showLocationRect.origin.x+showLocationRect.size.width/2, showLocationRect.origin.y-markImageView.frame.size.height, markImageView.frame.size.width, markImageView.frame.size.height)];
            [markImageView setImage:UPIMAGE];
            [markImageView setTransform:CGAffineTransformRotate(CGAffineTransformIdentity, M_PI)];
            [self addSubview:markImageView];
            CGFloat width = self.bounds.size.width-showLocationRect.origin.x-markImageView.frame.size.width;
            ...
            CGSize size = [markTextView sizeThatFits:CGSizeMake(width, height)];
            if ([[PronetwayYunFoodHandle shareHandle].dianpuright isEqualToString:@"dianpuright"]) {
                [markTextView setFrame:CGRectMake(markImageView.frame.origin.x-20*newKwith, markImageView.frame.origin.y-size.height+markTextView.font.pointSize, size.width, size.height)];
    ...
    else if (markCenter.x>=centerX&&markCenter.y>=centerY)//右下
    {
        ...
        {
            [markImageView setFrame:CGRectMake(showLocationRect.origin.x+showLocationRect.size.width/2-markImageView.frame.size.width, showLocationRect.origin.y-markImageView.frame.size.height, markImageView.frame.size.width, markImageView.frame.size.height)];
            [markImageView setImage:DOWNIMAGE];
            [markImageView setTransform:CGAffineTransformIdentity];
            [self addSubview:markImageView];
            CGFloat width = showLocationRect.origin.x+showLocationRect.size.width-markImageView.frame.size.width;
            CGFloat height = showLocationRect.origin.y-markImageView.frame.size.height;
            CGSize size = [markTextView sizeThatFits:CGSizeMake(width, height)];
            [markTextView setFrame:CGRectMake(markImageView.frame.origin.x-size.width+10*newKwith, markImageView.frame.origin.y-size.height+markTextView.font.pointSize, size.width, size.height)];
            [self addSubview:markTextView];
        }
    }
}
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        [markImageView setFrame:CGRectMake(0, 0, 70*newKwith, 70*newKhight)];
        ...
        [markTextView setFont:[UIFont systemFontOfSize:17.0f]];
        [markTextView setScrollEnabled:NO];
        [markTextView setText:self.markText];
        [markTextView setBackgroundColor:[UIColor clearColor]];
        
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(dianpuTapClick:)];
...
-(void)dianpuTapClick:(UITapGestureRecognizer *)tap {

    if (_delegate && [_delegate respondsToSelector:@selector(onprsee:superselef:)]) {
        
        [_delegate onprsee:tap superselef:self];
    }
