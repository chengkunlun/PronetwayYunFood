...
-(instancetype)initWithFrame:(CGRect)frame withTag:(NSInteger)tag
{
    ...
    if (self) {
      ...
      //  [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(showcell) name:@"showappcell" object:nil];
...
-(UIImageView *)RightarrowImageView
{
    if (!_RightarrowImageView) {
        ...
        _RightarrowImageView.backgroundColor = [UIColor redColor];
                   ...
                   sectionState:(YUFoldingSectionState)sectionState
{
    ...
    if (sectionState == YUFoldingSectionStateShow) {//展开
...
-(void)setupSubviewsWithArrowPosition:(YUFoldingSectionHeaderArrowPosition)arrowPosition
{
    ...
    //self.RightarrowImageView.frame = CGRectMake(0, kWidth-32-20, 32, 32);
    ...
    [UIView animateWithDuration:0.2
                     animations:^{
                         if (shouldExpand) {//展开
                             ...
                             if (self.arrowPosition == YUFoldingSectionHeaderArrowPositionRight) {
                                 _arrowImageView.transform = CGAffineTransformMakeRotation(M_PI/4);
                             }else{
                                 self.arrowImageView.transform = CGAffineTransformMakeRotation(0);
                             }
                         }
...
-(void)onTapped:(UITapGestureRecognizer *)gesture
{
    ...
    [kNotificationCenter postNotificationName:@"takeback" object:nil];
